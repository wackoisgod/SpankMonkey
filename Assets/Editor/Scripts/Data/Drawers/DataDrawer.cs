...
public abstract class DataDrawer
{
	...
	{
		if (property.GetCustomAttributes(typeof(NonEditable), true).Length == 0)
		{
			if (property.PropertyType.IsArray)
			{
				...
				if (drawer != null)
				{
					...
					drawerInstance?.Draw(value, property.Name, null, property, output => { property.SetValue(root, output, null); });
				}
				else
				{
					...
					if (arrayValue != null)
					{
						...
						for (int i = 0; i < arrayValue.Length; i++)
						{
							...
							DataDrawer drawerInstance = Activator.CreateInstance(drawer) as DataDrawer;
							drawerInstance?.Draw(element, i.ToString(), null, property, output =>
							{
	...
	public abstract void Draw(object input, string name, string tooltip, PropertyInfo property, Action<object> setValueCallback);
	...
	private static Type GetDrawer(PropertyInfo property, object value, bool useAttributes, Type defaultDrawer)
	{
		...
		if (useAttributes)
		{
			object[] drawerAttr = property.GetCustomAttributes(typeof(DataDrawAttribute), true);
